    ...
    container.innerHTML = `
      <div id="videosPage" class="flex flex-col flex-1 min-h-0">
        <div class="flex-1 overflow-y-auto p-6 space-y-4">
          <h2 class="text-lg font-semibold tracking-tight text-neutral-900">Generate Videos</h2>
          ...
          <div class="space-y-3">
            ...
            <div class="flex gap-3">
              <div class="flex-1">
                ...
                <select id="videoModel" class="w-full bg-white/60 border border-neutral-200/50 rounded-xl px-3 py-2.5 text-sm text-neutral-700 focus:bg-white/90 focus:outline-none transition-all shadow-sm">
              ...
              <div class="w-24">
                ...
                <select id="videoDuration" class="w-full bg-white/60 border border-neutral-200/50 rounded-xl px-3 py-2.5 text-sm text-neutral-700 focus:bg-white/90 focus:outline-none transition-all shadow-sm">
                </select>
              </div>
              <div class="w-28">
                <label class="text-xs font-medium text-neutral-400 uppercase tracking-wider mb-1.5 block">Aspect</label>
                <select id="videoAspect" class="w-full bg-white/60 border border-neutral-200/50 rounded-xl px-3 py-2.5 text-sm text-neutral-700 focus:bg-white/90 focus:outline-none transition-all shadow-sm">
            ...
            <button id="videoGenBtn"
              class="w-full px-4 py-2.5 rounded-lg bg-neutral-900 text-sm font-medium text-white hover:bg-neutral-800 transition-all shadow-sm disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2">
          ...
          <div id="videoResult" class="hidden">
            <div id="videoLoading" class="hidden text-center py-8">
              ...
              <div class="mt-3 w-48 mx-auto bg-neutral-100 rounded-full h-1.5 overflow-hidden">
            ...
            <div id="videoOutput" class="hidden">
              <video id="generatedVideo" controls class="w-full rounded-2xl border border-neutral-200/40 shadow-sm"></video>
          ...
          <!-- History -->
          <div>
            <h3 id="videoHistoryTitle" class="text-sm font-medium text-neutral-700 mb-2 hidden">Recent Videos</h3>
  ...
  async _loadHistory(container) {
    ...
    try {
      ...
      for (const item of items) {
        ...
        el.className = 'flex gap-3 p-3 rounded-2xl bg-white/50 border border-neutral-200/40 backdrop-blur-md hover:bg-white/80 transition-all items-center';
        el.innerHTML = `
          ...
          <div class="min-w-0 flex-1">
            <p class="text-xs text-neutral-700 truncate">${item.prompt || 'No prompt'}</p>
