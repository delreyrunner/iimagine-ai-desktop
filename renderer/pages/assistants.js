...
const AssistantsPage = {
    ...
    el.innerHTML = `
      <div class="p-6 space-y-4">
        <div class="flex items-center justify-between">
          <h2 class="text-lg font-semibold tracking-tight text-neutral-900">AI Assistants</h2>
          <button id="asstNewBtn" class="px-4 py-2.5 rounded-lg bg-neutral-900 text-sm font-medium text-white hover:bg-neutral-800 transition-all shadow-sm">+ New Assistant</button>
        </div>
        ...
        <div id="asstGrid" class="space-y-2">
          ...
          ` : assistants.map(a => `
            <div class="asst-item bg-white/50 border border-neutral-200/40 rounded-2xl p-5 shadow-[0_2px_10px_rgb(0,0,0,0.02)] backdrop-blur-md hover:shadow-md cursor-pointer transition-all" data-id="${a.id}">
              <div class="flex items-center justify-between">
                <div class="min-w-0 flex-1">
                  <h3 class="text-sm font-medium text-neutral-900">${this._esc(a.title)}</h3>
    ...
    el.innerHTML = `
      <div class="p-6 space-y-4">
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-lg font-semibold text-neutral-900">${isNew ? 'New Assistant' : 'Edit Assistant'}</h2>
        </div>
        ...
        <div class="space-y-3">
          ...
          <div>
            ...
            <textarea id="asstPrompt" rows="6" placeholder="You are a helpful assistant that..."
              class="w-full resize-none bg-white/60 border border-neutral-200/50 rounded-xl px-3 py-2.5 text-sm text-neutral-700 font-mono focus:bg-white/90 focus:outline-none transition-all shadow-sm">${this._esc(assistant?.system_prompt || '')}</textarea>
          </div>
          <div>
            <label class="text-xs font-medium text-neutral-400 uppercase tracking-wider mb-1.5 block">Knowledge Base (optional)</label>
            <select id="asstCollection" class="w-full bg-white/60 border border-neutral-200/50 rounded-xl px-3 py-2.5 text-sm text-neutral-700 focus:bg-white/90 focus:outline-none transition-all shadow-sm">
        ...
        <div class="flex gap-2 pt-2">
          <button id="asstSaveBtn" class="px-6 py-2.5 rounded-lg bg-neutral-900 text-sm font-medium text-white hover:bg-neutral-800 transition-all shadow-sm">${isNew ? 'Create' : 'Save'}</button>
          <button id="asstCancelBtn" class="text-sm text-neutral-500 px-4 py-2 hover:text-neutral-700">Cancel</button>
        </div>
      </div>
    `;
  ...
  async _showChat(assistantId) {
    ...
    el.innerHTML = `
      <div class="flex flex-col flex-1 min-h-0 h-full">
        <div class="flex items-center gap-2 px-4 py-3 border-b border-neutral-200/40 flex-shrink-0">
          <button id="asstChatBack" class="text-neutral-400 hover:text-neutral-600 text-sm">← Back</button>
          <span class="text-neutral-300">|</span>
          <div class="min-w-0 flex-1">
            <h3 class="text-sm font-medium text-neutral-900 truncate">${this._esc(assistant.title)}</h3>
          ...
          <button id="asstNewConvBtn" class="px-4 py-2 rounded-lg bg-white/60 border border-neutral-200/50 text-sm font-medium text-neutral-700 hover:bg-white/90 transition-all shadow-sm flex items-center gap-2"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M8 12h8M12 8v8"/></svg> New Chat</button>
        </div>

        <div id="asstMessages" class="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          ${messages.length === 0 ? `
            <div class="text-center py-8 text-neutral-400">
              <div class="p-3 bg-white rounded-2xl border border-neutral-100 shadow-sm text-neutral-400 inline-flex mb-2"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></div>
        ...
        <div class="px-4 py-3 border-t border-neutral-200/40 flex-shrink-0">
          <div class="flex gap-2">
            <textarea id="asstInput" rows="3" placeholder="Type a message..."
              class="flex-1 bg-white/60 border border-neutral-200/50 rounded-xl px-3 py-2.5 text-sm text-neutral-700 placeholder-neutral-400 focus:bg-white/90 focus:outline-none transition-all shadow-sm resize-none"></textarea>
            <button id="asstSendBtn" class="px-4 py-2.5 rounded-lg bg-neutral-900 text-sm font-medium text-white hover:bg-neutral-800 transition-all shadow-sm disabled:opacity-40 self-end">Send</button>
          </div>
        </div>
      </div>
    `;
  ...
  _bindChatEvents(assistant) {
    ...
    const sendMessage = async () => {
      ...
      msgContainer.innerHTML += `<div id="${responseId}" class="message-enter bg-white/50 border border-neutral-200/40 rounded-2xl px-4 py-3 text-sm text-neutral-800 backdrop-blur-md max-w-[80%] whitespace-pre-wrap"><span class="text-neutral-400">Thinking...</span></div>`;
      msgContainer.scrollTop = msgContainer.scrollHeight;
  ...
  _renderMessage(role, content) {
    if (role === 'user') {
      return `<div class="message-enter flex justify-end"><div class="bg-neutral-900 text-white rounded-lg px-3 py-2 text-sm max-w-[80%]">${this._esc(content)}</div></div>`;
    }
    if (role === 'system') return '';
    return `<div class="message-enter bg-white/50 border border-neutral-200/40 rounded-2xl px-4 py-3 text-sm text-neutral-800 backdrop-blur-md max-w-[80%] whitespace-pre-wrap">${this._esc(content)}</div>`;
  },
